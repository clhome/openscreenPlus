      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[60] bg-[#09090b] rounded-2xl shadow-2xl border border-white/10 p-8 w-[90vw] max-w-md animate-in zoom-in-95 duration-200">
              ...
              <>
                ...
                <div>
                  <span className="text-xl font-bold text-slate-200 block">
                    {error ? t('editor.exportFailed') : isExporting ? t('export.exporting') : t('editor.exportVideo')}
                  </span>
                  <span className="text-sm text-slate-400">
                    {error ? t('export.tryAgain') : isExporting ? t('export.pleaseWait') : t('export.readyToStart')}
                  </span>
                </div>
              </>
            )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">{t('export.status')}</div>
        ...
        {showSuccess && (
          <div className="text-center py-4 animate-in zoom-in-95">
            <p className="text-lg text-slate-200 font-medium">{t('export.savedSuccess')}</p>
          </div>
        )}
      </div>
    </>
