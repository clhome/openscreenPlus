...
export default function TimelineEditor({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // 阻止这些按键的默认行为
      if (['KeyF', 'KeyZ', 'KeyX', 'KeyC', 'Tab'].includes(e.code)) {
        if (e.code === 'KeyZ' && (e.ctrlKey || e.metaKey)) return; // 允许 Ctrl+Z
        if (e.code === 'KeyX' && (e.ctrlKey || e.metaKey)) return; // 允许 Ctrl+X
        if (e.code === 'KeyC' && (e.ctrlKey || e.metaKey)) return; // 允许 Ctrl+C
        e.preventDefault();
      }
      if (e.code === 'Delete' || e.code === 'Backspace') {
        // e.preventDefault(); // Delete usually doesn't need preventDefault unless focused on inputs, which we handle above
      }
    };
